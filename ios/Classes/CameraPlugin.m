...
previewPhotoSampleBuffer:(CMSampleBufferRef)previewPhotoSampleBuffer
    ...
    // Temparory fix request OCR request by File Failure, save display and request image separately
    UIImage *image = [self fixOrientationForImage:originalImage];
    UIImage *displayImage = [UIImage imageWithCGImage:originalImage.CGImage
                                         scale:1.0
                                   orientation:[self getImageRotation]];
    
    [UIImageJPEGRepresentation(displayImage, 1.0) writeToFile:[_path stringByReplacingOccurrencesOfString:@"request.jpeg" withString:@"display.jpeg"]
           atomically:YES];
...
@property(readonly, nonatomic) AVCaptureDeviceInput *captureVideoInput;
...
- (instancetype)initWithCameraName:(NSString *)cameraName
    ...
    if (@available(iOS 13.0, *)) {
        [_capturePhotoOutput setMaxPhotoQualityPrioritization:AVCapturePhotoQualityPrioritizationSpeed];
    }
...
CameraPlugin *currentCameraPluginInstance = nil;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
    ...
    CameraPlugin *instance = [[CameraPlugin alloc] initWithRegistry:[registrar textures]
                                                          messenger:[registrar messenger] chanel: channel];
    [registrar addMethodCallDelegate:instance channel:channel];
    currentCameraPluginInstance = instance;
}

+ (CameraPlugin*) getCurrentInstance {
    return currentCameraPluginInstance;
}

- (instancetype)initWithRegistry:(NSObject<FlutterTextureRegistry> *)registry
                       messenger:(NSObject<FlutterBinaryMessenger> *)messenger
                          chanel:(FlutterMethodChannel* )chanel{
    self = [super init];
    NSAssert(self, @"super init cannot be nil");
    _channel = chanel;
    _registry = registry;
